...
def _detection_loss(cls_outputs, box_outputs, labels, loss_vals, in_model,
    ...
    for level in levels:
      ...
      class_loss_layer = FocalLoss(alpha, gamma, label_smoothing, reduction=tf.keras.losses.Reduction.NONE)
      if class_loss_layer:
      ...
      box_loss_class = BoxLoss(delta, reduction=tf.keras.losses.Reduction.NONE)
    ...
    if iou_loss_type:
      ...
      box_iou_loss_layer = BoxIouLoss(iou_loss_type, min_level, max_level, 
...
def train_step(in_model, data, optimizer, steps_per_execution, weight_decay, clip_gradients_norm, var_freeze_expr,
    ...
    for _ in tf.range(steps_per_execution) :
      ...
      # return loss_vals
      outfeed_queue.enqueue(loss_vals['loss'])
...
class EfficientDetNetTrain(efficientdet_keras.EfficientDetNet):
  ...
  def _detection_loss(self, cls_outputs, box_outputs, labels, loss_vals):
    ...
    total_loss = (
        ...
        config.iou_loss_weight * box_iou_loss)
    loss_vals['det_loss'] = total_loss
    loss_vals['cls_loss'] = cls_loss
    loss_vals['box_loss'] = box_loss
    return total_loss
